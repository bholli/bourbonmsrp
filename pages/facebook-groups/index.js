...
const Page = ({groups}) => (
  <>
    ...
    <Head>
        ...
        <title>Bourbon Groups on Facebook - Bourbon MSRP</title>
    ...
    <Layout>
      <div>
        ...
        {groups.map(group => (
          <div>
            <Link href='/facebook-groups/[id]' as={`/facebook-groups/${group.id}`}>
              <a>{group.name}</a>
            </Link>
          </div>
        ))}
...
Page.getInitialProps = async function(){
    const response = await fetch('https://api.bourbonspotter.com/facebook-groups');
